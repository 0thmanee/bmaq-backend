...
module.exports = async function (fastify: FastifyInstance) {
  ...
  fastify.get('/requests/list', {
    ...
    try {
      const { status = 'all', category = 'all', search = '', limit = 50 } = request.query as {
        status?: string;
        category?: string;
        search?: string;
        limit?: number;
      ...
      if (search) {
        whereClause.OR = [
          ...
          { startup: { company_name: { contains: search } } },
  ...
  fastify.post('/requests/:id/decision', {
    ...
    try {
      ...
      const { action, reason, comments } = request.body as ApprovalForm;
  ...
  fastify.get('/requests/user/list', {
    ...
    try {
      const { status = 'all', category = 'all', limit = 50 } = request.query as any;
      const user = (request as any).user;
  ...
  fastify.post('/requests/user/create', {
    ...
    try {
      const user = (request as any).user;
      const { category, description, amount, priority } = request.body as any;
